...
import type { TargetModifier } from "./targets";
...
function applyMatchStateOutcome(
  current: MatchState,
  ...
  const territoryGain = Math.round(
    outcome.gain * 0.9 + (objectiveResult.success ? 5 : -4),
  );
  const possessionDelta = objectiveResult.success ? 5 : -7;
  const chanceDelta = Math.round(
    outcome.gain * 1.2 +
      objectiveResult.momentumDelta * 0.7 +
      (momentum - 50) / 7,
  );
  const pressureBonus =
    pressure.id === "red-zone-squeeze"
      ? 7
      : pressure.id === "passive-line"
        ? 3
        : 0;
  ...
  let next: MatchState = {
    ...
    scoringChance: clamp(
      current.scoringChance +
        chanceDelta +
        pressureBonus +
        (targetModifier?.chanceDelta ?? 0),
    ),
  };
